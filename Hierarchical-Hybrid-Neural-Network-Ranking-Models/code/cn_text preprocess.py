...
def doc_split(text_data):
    ...
    for sent in text:
        if len(sent) > 1:
            ...
            sent_cut = thu1.cut(sent,text=False) 
            word_list = [i[0].strip() for i in sent_cut]
...
data_df.to_csv(r'data_collect\CLT\data_clean.csv', index=False)
...
## Remove stop words from text with good word segmentation
def del_stopWord(text_data):
    text_list = text_data.split(' ')
    ## Remove stop words and extra spaces
    text_list = [word.strip() for word in text_list if word not in stop_words.text.values]
    ## The processed words are concatenated into strings using spaces
    text = " ".join(text_list)
    return text


data_df['del_stopWord'] = data_df.split.apply(del_stopWord)



data_df.to_csv(r'data_collect\CLT\data_split.csv', index=False)
